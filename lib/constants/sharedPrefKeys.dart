const String fName = 'fName';
const String lName = 'lName';
const String gender = 'gender';
const String age = 'age';
const String phoneNo = 'phoneNo';
const String emailId = 'emailId';
const String authkey = 'authkey';
const String isLoggedIn = 'login';

const String takeAPicImg = 'takeAPicImg';
const String takeAPicUrl = 'takeAPicUrl';
const String takeAPicText = 'takeAPicText';

const String dentHealthImg = 'dentHealthImg';
const String dentHealthUrl = 'dentHealthUrl';
const String dentHealthText = 'dentHealthText';
